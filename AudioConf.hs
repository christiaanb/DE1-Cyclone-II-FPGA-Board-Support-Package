...
audioConfig = mealyB audioConfigT audioConfInit
...
audioConfigT s@(AudioConfS{..}) inp = (s', outp)
  where
    (rst,ena,cmdAck,pack -> rxAck,al) = inp
    outp = (start,stop,write,v2bv din,done,fault)

    i2cSlvAddr = 0x34 :: BitVector 8
    ...
    s' = if rst then
        ...
        case audioConfStateM of
          ...
          AUDIOCONFaddr -> s {audioConfStateM = AUDIOCONFaddrAck, start = True, write = True, din = bv2v i2cSlvAddr}
          ...
          AUDIOCONFreg -> if (rxAck == low) then -- trace "Add Acknowledged" $
              s {audioConfStateM = AUDIOCONFregAck, write = True, din = bv2v (fst lutData), fault = False}
            else
